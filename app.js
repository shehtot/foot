...
function listenToDraft() {
    ...
    // 3. الاستماع لقائمة الكباتن لعرضها في الجدول اللحظي
    db.ref('captains').on('value', (snapshot) => {
        const adminCapUl = document.getElementById('admin-captains-list');
        if (adminCapUl) {
            adminCapUl.innerHTML = '';
            snapshot.forEach((child) => {
                const capData = child.val();
                const capId = child.key;
                const li = document.createElement('li');
                
                // === هذا هو السطر المسؤول عن إظهار الاسم وبجانبه الباسورد ===
                li.innerHTML = `<span>👨‍✈️ ${capData.name} <small>(${capData.phone})</small></span> 
                                <button class="delete-item-btn" onclick="removeCaptain('${capId}')">❌ حذف</button>`;
...
function resetTeamsOnly() {
    if(confirm("هل أنت متأكد؟ سيتم مسح ترتيب الكباتن الحالي وجميع اختيارات اللاعبين للفرق، والبدء من جديد؟")) {
        // مسح الفرق
        db.ref('teams').remove();
        // مسح حالة التقسيمة (الدور الحالي والترتيب)
        db.ref('draftState').remove();
        
        // إعادة اللاعبين لحالة "لم يتم الاختيار"
        db.ref('players').once('value').then(snapshot => {
            snapshot.forEach(child => {
                db.ref(`players/${child.key}`).update({ isPicked: false });
            });
        });
        
        alert("تمت إعادة ضبط التقسيمة بنجاح، يمكنك الآن البدء من جديد.");
    }
}